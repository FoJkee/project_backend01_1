...
export const postMaddleware = [
    ...
    body('blogId').exists().bail().isString().bail().trim().isLength({min: 1, max: 100}).withMessage('Incorrect blogId'),
    body('blogName').exists().bail().isString().bail().trim().isLength({min: 1, max: 100}).withMessage('String length is not more than 100 symbols')
